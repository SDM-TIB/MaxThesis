...
def transform_predicate_with_object(kg: Graph, prefix) -> Tuple[Graph, Dict[str, str]]:
    ...
    # collect original predicates, used for rule mining
    original_predicates = set()
    ...
    return kg_transform, predicate_mapping, original_predicates
...
def transform(kg: Graph, constraints_folder: str, prefix,  kg_name: str = None) -> Tuple[Graph, str]:
    ...
    try:
        ...
        kg_transform.serialize(destination=initial_transform_file, format='nt')
        print(f"Saved initial transformed KG to {initial_transform_file}")

        # Setup for constraint validation
        constraints_result_dir = f"{constraints_folder}/result_{kg_name}"
        shapes_file = f"{constraints_folder}/{kg_name}.ttl"
        validation_report = f"{constraints_result_dir}/validationReport.ttl"
        ...
        print(f"Original triples: {len(list(kg))}")
        print(f"Initially transformed triples: {len(list(kg_transform))}")
        print(f"Final transformed triples: {len(list(transformed_kg))}")
        print(f"Violations processed: {len(violations)}")

        #MAX save predicate mappings to file
        print(f"Saving predicate mappings (transformed to original) to {output_dir}/predicate_mappings.json...")
        ...
        return transformed_kg, output_dir, original_predicates

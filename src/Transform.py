...
def add_types_to_nt(nt, types, out, prefix):
        with open(nt, 'r', encoding='utf-8') as nt, open(types, 'r', encoding='utf-8') as types, open(out, 'w', encoding='utf-8') as out:
            ...
            for line in nt:
                out.write(line)
                s = line.split(" ")
                entities.add(s[0].removesuffix(">").removeprefix(f"<{prefix}"))
                entities.add(s[2].removesuffix(">").removeprefix(f"<{prefix}"))
            ...
            for line in types:
                ...
                if s[0] in entities:
                    ...
                    c += 1
                    out.write(f"<{prefix}{s[0]}> <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <{prefix}{s[2]}> .\n")
            print(c)
...
def add_classes_to_ontology(tax, on, out):
    with open(tax, 'r', encoding='utf-8') as tax, open(on, 'r', encoding='utf-8') as on, open(out, 'w', encoding='utf-8') as out:
        classes = {"wordnet_person_100007846", "wordnet_organization_108008335", "wordnet_location_100027167", "wordnet_language_106282651", "wordnet_city_108524735", "wordnet_university_108286569",
                   "yagoLegalActor", "owl:Thing","wordnet_sex_105006898","yagoPermanentlyLocatedEntity","yagoGeoEntity","rdfs:Class","wordnet_actor_109765278",
                   "wordnet_movie_106613686","wordnet_event_100029378","yagoLegalActorGeo","wordnet_award_106696483","wordnet_editor_110044879","wordnet_country_108544813",
                   "wordnet_organization_108008335", "wordnet_administrative_district_108491826", "yagoURL"}
        prefix = "yago:"
        for line in on:
            out.write(line)
        new_classes = set()
        ...
        while classes:
            for line in tax:
                ...
                if s[0] in classes or s[2] in classes:
                    out.write(f"{prefix}{s[0]} {s[1]} {prefix}{s[2]} .\n")
                    if s[0] not in classes:
                        new_classes.add(s[0])
                    if s[2] not in classes:
                        new_classes.add(s[2])
        ...
        for c in old_classes:
            out.write(f"yago:{c} rdf:type rdfs:Class .\n")


if __name__== '__main__':
    add_classes_to_ontology(".\Data\KG\YAGO3-10\yagoTaxonomy.nt",".\Data\Ontology\YAGO3-10-properties.ttl",".\Data\Ontology\YAGO3-10Ontology.ttl")

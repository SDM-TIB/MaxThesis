...
def add_classes_to_ontology(tax, on, out):
    with open(tax, 'r', encoding='utf-8') as tax, open(on, 'r', encoding='utf-8') as on, open(out, 'w', encoding='utf-8') as out:
        ...
        new_classes = set()
        old_classes = set()
        # !!!!!!!!! mistakes here, does not get all connected triples as expected
...
def yago_tsv_to_nt(inf, outf):
    with open(inf, 'r', encoding='utf-8') as inf, open(outf, 'w', encoding='utf-8') as outf:
        for line in inf:
            l = line.split("\t")
            outf.write(f"<{l[0]}> <{l[1]}> <{l[2].removesuffix("\n")}>\n")


if __name__== '__main__':
    pyclause_rules_to_csv(".\Data\Rules\FrenchRoyalty-AMIE.txt",".\Data\Rules\FrenchRoyalty-AMIE.csv")
